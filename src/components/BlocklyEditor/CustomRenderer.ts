...
class CustomConstantProvider extends Blockly.blockRendering.ConstantProvider {
  // 参考文档：https://blocklycodelabs.dev/codelabs/custom-renderer/index.html?index=..%2F..index#0
  RECT_PREV_NEXT!: { width: number; height: number; pathLeft: string; pathRight: string }
  RECT_INPUT_OUTPUT!: { width: number; height: number; pathUp: string; pathDown: string }
  constructor() {
...
export class CustomRenderer extends Blockly.blockRendering.Renderer {
   ...
   */
  makeConstants_() {
    return new CustomConstantProvider() as any
  }
}
