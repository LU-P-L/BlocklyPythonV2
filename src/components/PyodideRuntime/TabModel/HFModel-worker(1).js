...
const logger = log4TSProvider.getLogger('HFModel-worker');

env.useBrowserCache = true;
env.remoteHost = 'http://localhost:4000';
env.remotePathTemplate = "data/{model}/";
env.backends.onnx.wasm.wasmPaths = 'http://localhost:4000/data/ort-wasm/';
...
async function check() {
  logger.info("检查是否支持WebGPU");
  try {
    const adapter = await navigator.gpu.requestAdapter();
    if (!adapter) {
      throw new Error("您的电脑不支持WebGPU");
    }
...
class TextGenerationPipeline {
  static async getInstance(model_id, progress_callback = null) {
    logger.info("获取模型实例", env);
    console.log("TextGenerationPipeline", model_id);
...
async function generate(model_id, messages) {
  logger.info("使用模型生成文本", model_id, messages);
...
self.addEventListener("message", async (e) => {
  ...
  switch (type) {
    ...
    case "generate":
      ...
      generate(data.model_id, data.messages);
      break;
