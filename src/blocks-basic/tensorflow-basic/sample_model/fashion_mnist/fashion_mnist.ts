...
export function addFashionMnistBlocks(Blockly: any, pythonGenerator: any, workspaceSvg: any) {
    //导入TF相关库
    Blockly.Blocks['import_tf_base2'] = {
        init: function () {
...
`
    ...
    //导入数据集
    Blockly.Blocks['import_fashion_mnist_data'] = {
        init: function () {
    ...
    pythonGenerator.forBlock['import_fashion_mnist_data'] = function () {
       ; (window as any).blockly_tensorflow = (window as any).blockly_tensorflow || {}
       // 将扁平化的图像数据转换为28x28像素的二维数组 
       //;(window as any).flattenTo2D = function (flatArray, width, height) {
      //  function trainflattenTo2D() {  
      //   const twoDArray = [];  
      //   for (let i = 0; i < trainData.train_images.length; i += 28) {  
      //     const row = trainData.train_images.slice(i, i + 28);  
      //     twoDArray.push(row);  
      //   }  
      //   // 将二维数组转换为三维数组（样本数, 高度, 宽度）  
      //   const threeDArray = [];  
      //   for (let i = 0; i < twoDArray.length / 28; i++) {  
      //     const start = i * 28;  
      //     const end = start + 28;  
      //     threeDArray.push(twoDArray.slice(start, end));  
      //   }  
      //   return threeDArray;  
      // }  
        
      //  function testflattenTo2D() {  
      //     const twoDArray = [];  
      //     for (let i = 0; i < testData.test_images.length; i += 28) {  
      //       const row = testData.test_images.slice(i, i + 28);  
      //       twoDArray.push(row);  
      //     }  
      //     // 将二维数组转换为三维数组（样本数, 高度, 宽度）  
      //     const threeDArray = [];  
      ...
      //   }  
      // // 转换训练集数据  
      // const trainImages = trainflattenTo2D();  
      // const trainLabels = trainData.train_labels;  
      // const testImages = testflattenTo2D();  
      // const testLabels = testData.test_labels;  

      //  ;(window as any).blockly_dataset.trainImagesData = function () {
      //   const trainImages_ = trainImages;
      //   //const trainImage = trainImages.reshape(-1, 28, 28) 
      //   console.log(trainImages_)
      //   return  trainImages_
       
      //  }
      //  ;(window as any).blockly_dataset.trainLabelsData = function () {
      //   const trainLabels_ = trainLabels;  
      //   console.log(trainLabels_)
      //   return  trainLabels_
        
      //  }
      //  ;(window as any).blockly_dataset.testImagesData = function () {
      //   const testImages_ = testImages; 
      //   console.log(testImages_)
      //   return  testImages_
      //  }
      //  ;(window as any).blockly_dataset.testLabelsData = function () {
      //   const testLabels_ = testLabels; 
      //   console.log(testLabels_)
      //   return  testLabels_
      //  }
        return `
# 读取训练数据
with open('/data/local/fashion_mnist_train2.json', 'r') as f:
  train_data_json = json.load(f)

# 读取测试数据
with open('/data/local/fashion_mnist_test2.json', 'r') as f:
...
# 将展平的图像数据转换为三维数组 (样本数, 高度, 宽度)  
train_images = train_images_flat.reshape(-1, 28, 28)  
test_images = test_images_flat.reshape(-1, 28, 28)
print(train_images)
print(test_images)
#(train_images, train_labels)=(blockly_dataset.trainImagesData(),blockly_dataset.trainLabelsData())
#(test_images, test_labels) = (blockly_dataset.testImagesData(),blockly_dataset.testLabelsData())
...
#test_labels = np.array(test_labels)\n`
    ...
    //加载数据
    Blockly.Blocks['load_fashion_mnist_data'] = {
        init: function () {
            this.setPreviousStatement(true, null)
            this.setNextStatement(true, null)
            this.setColour(FASHION_COLOR)
    ...
    pythonGenerator.forBlock['load_fashion_mnist_data'] = function () {
        ; (window as any).blockly_tensorflow = (window as any).blockly_tensorflow || {}
        return `
class_names = ['T-shirt/top', 'Trouser', 'Pullover', 'Dress', 'Coat','Sandal', 'Shirt', 'Sneaker', 'Bag', 'Ankle boot']\n`
    ...
    //预处理数据
    Blockly.Blocks['deal_fashion_mnist_data'] = {
        init: function () {
            this.setPreviousStatement(true, null)
            this.setNextStatement(true, null)
            this.setColour(FASHION_COLOR)
    ...
    pythonGenerator.forBlock['deal_fashion_mnist_data'] = function () {
        ; (window as any).blockly_tensorflow = (window as any).blockly_tensorflow || {}
        return `
plt.figure()
...
plt.figure(figsize=(10,10))\n`
    ...
    //构建模型
    Blockly.Blocks['create_fashion_mnist_moudle'] = {
        init: function () {
    ...
    pythonGenerator.forBlock['create_fashion_mnist_moudle'] = function (block: any) {
        ; (window as any).blockly_tensorflow = (window as any).blockly_tensorflow || {}
...
//         `
    ...
    Blockly.Blocks['tf_fashion_mnist_train'] = {
        init: function () {
      ...
      pythonGenerator.forBlock['tf_fashion_mnist_train'] = function () {
        return `
# 训练
model.fit(train_images, train_labels, epochs=10)
    `
      ...
      //评估准确率
      Blockly.Blocks['tf_fashion_mnist_ssessment'] = {
        init: function () {
      ...
      pythonGenerator.forBlock['tf_fashion_mnist_ssessment'] = function () {
        return `
# 评估准确率
...
print('Test accuracy:', test_acc)
    `
      ...
      //进行预测
      Blockly.Blocks['tf_fashion_mnist_Forecast'] = {
        init: function () {
      ...
      pythonGenerator.forBlock['tf_fashion_mnist_Forecast'] = function () {
        return ``
//         return `
// # 进行预测
...
//     `
      ...
      //输出预测结果
      Blockly.Blocks['tf_fashion_mnist_print'] = {
        init: function () {
      ...
      pythonGenerator.forBlock['tf_fashion_mnist_print'] = function () {
        return ``
//         return `
// # 输出第0个图像预测结果
// i = 0
