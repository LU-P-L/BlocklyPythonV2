...
class AdditionRNNDemo {
  ...
  async train(iterations, batchSize, numTestExamples) {
    ...
    for (let i = 0; i < iterations; ++i) {
      ...
      tfvis.render.table(tableSurface, { headers, values }, { fontSize: 15 })
    }
  }
}

Blockly.Blocks['addition_rnn'] = {
  init: function () {
    this.setPreviousStatement(false, null)
    this.setNextStatement(true, null)
    this.setColour(RNN_COLOR)
    this.appendDummyInput('dummyInput').appendField('加法RNN')
  }
}

pythonGenerator.forBlock['addition_rnn'] = function () {
  const digits = 2
  const trainingSize = 5000
  const rnnType = 'SimpleRNN'
  const layers = 1
  const hiddenSize = 128
  const batchSize = 128
  const trainIterations = 100
  const numTestExamples = 20
  ...
  window.blockly_tensorflow.additionRNN = async function () {
    console.log('--------------------begin----------------------\n')
    await demo.train(trainIterations, batchSize, numTestExamples)
    console.log('--------------------end----------------------\n')
  }

  return `from js import blockly_tensorflow
blockly_tensorflow.additionRNN()\n`
}
