...
export function addTFDatasetReutersWordIndex(Blockly: any, pythonGenerator: any, workspaceSvg: any) {

    const dataset_name = "reuters_word_index";
    const dataset_fetch_url = `tf_keras_dataset/${dataset_name}.json`;
    const dataset_fs_path = `/${dataset_name}.json`;
    ...
    Blockly.Blocks["tf_dataset_reuters_get_word_index"] = {
        init: function () {
            let that = this;
    ...
    pythonGenerator.forBlock["tf_dataset_reuters_get_word_index"] = function (block) {
        pythonGenerator.definitions_["import json"] = "import json";
        return [`json.load(open("${dataset_fs_path}"))`, pythonGenerator.ORDER_NONE];
        ;
    }
}